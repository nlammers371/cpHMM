...
R_equal_corners = np.array([[-.008, .015 * corr, .015],
                            [.004, -.019 * corr, .015],
                            [.004, .004 * corr, -.03]]) * dT * 1.25

R_small_corners = np.array([[-.008, .015 * corr, .006],
                            [.0064, -.019 * corr, .024],
                            [.0016, .004 * corr, -.03]]) * dT * 1.25

R_slow = np.array([[-.008, .015*corr, 0.0],
                   [.008, -.019*corr, .03],
                   [0.0, .004*corr, -.03]]) * dT

R_fast =  np.array([[-.008, .015 * corr, 0.0],
                   [.008, -.019 * corr, .03],
                   [0.0, .004 * corr, -.03]]) * dT * 1.5

R_2state_slow = np.array([[-.008, .019],
                           [.008, -.019]]) * dT

R_2state_fast = np.array([[-.008, .019],
                           [.008, -.019]]) * dT * 1.5


#---------------------------------------Classes to Organize and  track Exp Vars----------------------------------------#
...
class Eve2ExpShort(object):
    def __init__(self):
        ...
        if num_states == 3:
            self.R = eval(r_type)
            if len(self.R[0,:])!=3:
                print("Warning: Size of specified rate matrix incompatible with state count")
                sys.exit(1)
...
if __name__ == "__main__":
    ...
    for routine in xrange(n_routines):
        ...
        with open(os.path.join(writepath, 'best_results.csv'), write) as inf_out:
            writer = csv.writer(inf_out)
            ...
            row = list(chain(*[[routine], [max_id],A_flat, R_flat, v_best.tolist(), [best_results[2]], [best_results[3]], expClass.pi,
        ...
        with open(os.path.join(writepath, 'full_results.csv'), write) as full_out:
            ...
            for n in xrange(RoutineParamsFinal.n_inf):
                ...
                try:
                    ...
                    continue
                row = list(chain(
                    *[[routine],[n], A_flat, R_flat, inf_results[n][1].tolist(), [inf_results[n][2]], [inf_results[n][3]], expClass.pi,
        ...
        with open(os.path.join(writepath, 'initializations.csv'), write) as init_out:
            ...
            for n in xrange(RoutineParamsFinal.n_inf):
                ...
                row = list(chain(*[[routine],[n], A_flat, R_flat, inf_results[n][1].tolist(), [inf_results[n][2]], expClass.pi]))
                writer.writerow(row)
        # If desired, print probability arrays from final step to csv
        if RoutineParamsFinal.keep_probs and RoutineParamsFinal.model == 'bw':
            s_array = best_results[7]
            with open(os.path.join(writepath, 'probability_distibutions.csv'), write) as init_out:
                writer = csv.writer(init_out)
                for n in xrange(n_traces):
                    for r in xrange(len(s_array[n][:,0])):
                        row = list(chain(*[[routine],[n],np.exp(s_array[n][r,:].tolist())]))
                        writer.writerow(row)
